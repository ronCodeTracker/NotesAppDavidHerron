...
export default class FSNotesStore extends AbstractNotesStore {
    ...
    async read(key) {
        ...
        console.log("2nd Key: 88888888888888888-9999999999999999" + key);
    ...
    async keylist() {
        ...
        const thenotes = filez.map(async fname => {
            const key9 = path.basename(fname, '.json');
            debug(`About to READ ${key9}`);
            const thenote = await readJSON(notesdir, key9);
            console.log("3333333333333333333333333 thenote: " + thenote.key);
            return thenote.key;
        });
...
async function notesDir() {
    ...
    console.log("Directory directory directory: " + dir);
...
async function readJSON(notesdir, key) {
    console.log("keySeven keySeven keySeven keySeven keySeven keySeven keySeven keySeven: " + key);
    const readFrom = filePath(notesdir, key);
    console.log("REad from ffreadfrom read from 55555555555555555555555555" + readFrom);
    const data = await fs.readFile(readFrom, 'utf8');
    console.log("log log log log log log");
    debug(`readJSON ${data}`);
    console.log("log log log log log log");
